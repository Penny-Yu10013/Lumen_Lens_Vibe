...
- 各標本（細節見 `docs/handoff/` 同名檔）：
  ...
  - 瑪瑙 iris：**M9b 厚牆 v4（2026-09-24，雲端 session 推分支，合併後 Pages 才更新）**＝第三輪批改：彩虹改成每板段每環一片暈、角向寬淡出、有機年輪、粗帶骨架、彩色處能量守恆；待第四輪批改
- M9b 瑪瑙厚牆 v4 已做；M9c 後處理/M9d 萬花筒未做（萬花筒緩議）。
...
陣容定位：達碧茲（生長結構）/星光（asterism）/李迪（色帶）/瑪瑙（繞射）/亞歷（chatoyancy+變色）＝五現象零重複。開發順序：達碧茲→星石→亞歷→李迪→瑪瑙（次序前三完成，剩李迪/瑪瑙切片組）。後續：瑪瑙 M9b 厚牆（v4 已做，待第四輪批改）→ 李迪厚牆 → P2 八面體（獨立 `octahedron.html`）。
