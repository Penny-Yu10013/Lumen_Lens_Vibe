...
- 各標本（細節見 `docs/handoff/` 同名檔）：
  ...
  - 瑪瑙 iris：**M9b 厚牆 v1 本機完成（2026-09-07，未 commit）**，probe 驗證過光源綁定/傾角掃/厚度/側壁；待用戶看成品截圖批改【人類決】板段數/環位置/飽和度。線上仍是 P0 薄切片
- M9b 瑪瑙厚牆 v1 已做（本機）；M9c 後處理/M9d 萬花筒未做（萬花筒緩議）。
...
陣容定位：達碧茲（生長結構）/星光（asterism）/李迪（色帶）/瑪瑙（繞射）/亞歷（chatoyancy+變色）＝五現象零重複。開發順序：達碧茲→星石→亞歷→李迪→瑪瑙（次序前三完成，剩李迪/瑪瑙切片組）。後續：瑪瑙 M9b 厚牆（v1 已做，待批改）→ 李迪厚牆 → P2 八面體（獨立 `octahedron.html`）。
