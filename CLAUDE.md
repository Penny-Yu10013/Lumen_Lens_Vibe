...
| 亞歷山大貓眼（id:'alex'） | 本體＝sliceLinear `#ifdef SLICE_ALEX`：**Cr³⁺ 雙透射窗必須分 4 頻帶採樣**（650/583/515/455 過 `spectral()` 合成——RGB 三通道塞不下「紅綠都透、黃吸收」）；吸收固定、光源權重 `uWarm` 0/1 切換（日光/燭光**兩鍵瞬切不做滑桿**【人類決】，`#lightSrcRow` 限 alex 顯示，hash `warm=1`）；光源權重差距要敢拉、燭光藍端補超物理量才會紫紅不橘棕（踩過兩輪）。貓眼＝DOME `#ifdef SLICE_ALEX`：星石機制單方向版，v3 批改後＝**雙層帶**（核心線 K45000×2 冷白＋外暈 K600×0.38）＋縱向低頻調變＋**milk & honey**（分側與帶用**同一個 ua**、光側符號＝`keyL.x` **每幀常數**——用每像素 H.x 會生出直線弦切「雙色皮球」，踩過；過渡 ±0.30、對比 0.8/1.2）＋放射狀邊緣壓暗＋受光半球 gate。**無中心光點**（貓眼不出曝光點【人類決】：ptA 拆除＋通用 front sheen/spec ×0.22）。絲光條紋振幅 0.08 別拉高（會變木紋珠）。科研依據＝`亞歷山大貓眼_科研報告.md`（gitignore） |
