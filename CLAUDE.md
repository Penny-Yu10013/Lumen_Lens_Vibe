...
- 各標本（細節見 `docs/handoff/` 同名檔）：
  ...
  - 瑪瑙 iris：**M9b 厚牆 v3 已推（2026-09-07，8cc776f，Pages 線上＝v3）**＝v2 幾何（曲線裂紋分區/側壁）＋第二輪批改（透明體＋選擇性染色＋彩虹層：本體 ≥光桌 60%、裂紋只留錯位、粗帶軟漸層、彩虹跨帶柔化去黑縫、出彩分塊、暖核），probe 全項驗證；待第三輪批改（用戶精神回來再發）
- M9b 瑪瑙厚牆 v3 已做（本機）；M9c 後處理/M9d 萬花筒未做（萬花筒緩議）。
...
**雲端 session**：`參考圖/`、完稿標準、科研報告、討論稿都 gitignore，不在本 repo → 在私人 repo `Penny-Yu10013/lumen_lens_refs`，開工先 `add_repo` 掛上再 clone（內容同本機檔名；含第三方圖片，**絕不可複製進本公開 repo**）。
...
陣容定位：達碧茲（生長結構）/星光（asterism）/李迪（色帶）/瑪瑙（繞射）/亞歷（chatoyancy+變色）＝五現象零重複。開發順序：達碧茲→星石→亞歷→李迪→瑪瑙（次序前三完成，剩李迪/瑪瑙切片組）。後續：瑪瑙 M9b 厚牆（v3 已做，待第三輪批改）→ 李迪厚牆 → P2 八面體（獨立 `octahedron.html`）。
